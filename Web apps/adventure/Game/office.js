...
var drawer = (function() {
	...
	function examine(gc) {
		if (hasVerb(drawer, "open")) {
			return "The drawer is closed."
		}
		else if (drawer.containedItems.indexOf(paperclip) > -1) {
			return "The drawer is, surprisingly enough, almost empty. The only thing you see is an old paper clip.";
		}
		else {
...
var houseHistoryBook = (function() {
	...
	function examine(gc) {
		if (flagIsSet(Flag.WallBroken, gc.flags)) {
			return "You examine the map. The entrance to the cellar turned out to be exactly where the book indicated.";
		}		
		else if (historySection.items.indexOf(historyBookShelf) > -1) {
			return "You examine the map. Behind the bookshelf on the eastern wall of the history section it seems like there has once been an entrance to the cellar."
		}
		else if (isCarried(gc, latinDictionary)) {
		    addToItems(historyBookShelf, historySection.items);
			setFlag(Flag.EntranceKnown, gc.flags);
			return "Using the latin dictionary, you are able to decipher the contents of the old book. It describes the history of the house in which the bookshop is located. Most of what is in the book you already know since your parents passed this information to you - whether as a child you wanted it or not - but you find a few nuggets of new information. The most interesting part is the fact that there used to be an entrance to a cellar from what is now the history section. Looking at the map, and reading the text surrounding it, you conclude that there must be a hidden entrance to the basement behind the eastern wall, currently covered by book shelves.";
		}
		else {
			return "The leather-bound old book is written in what you assume is latin. Although you may recognize a word here and there, you really have no way of telling what the book is about. You see a small map in one of the pages. You can't be sure but you have a slight feeling of recognition.";
		}
	}
})();
...
var safe = (function() {
	...
	function examine() {
		var door = hasVerb(safe, "close") ? "is open." : "has golden details, and a sign saying \"Samuel Withers & Co. Ltd. West Bromwich\". It has a dial for entering the correct combination and a large handle for opening the door."
		return "The old safe is painted black. Its heavy steel door " + door;
	}

	function enterCombination(gc, combination) {
		if (combination != null) {
			if (combination == "1979") {
				replaceVerb(safe, "enterCombination", "close");
				gc.result = "You enter the correct combination and with a light creaking, the door opens.";
			}
			else {
				gc.result = "For a moment there, you thought you remembered the code. A futile attempt."
			}
		}
		else {
			codeKeyboard.show(4, 
				code => {
					enterCombination(gc, code);
					game.update();
				});	 
		}
	}
	...
	function beforeOpen(gc) {
		if (that.codeEntered) {
			...
			that.codeEntered = true;
			return true;
		}
	}

	function afterOpen(gc) {
		gc.result = "You enter the correct combination and with a light creaking, the door opens.";
		isOpen = true;
	}*/
})();
...
var metalBox = (function() {
	...
	function getVerbs(gc) {
		if (!that.isPicked && isCarried(gc, paperclip)) {
	...
	function examine(gc) {
		if (that.isPicked) {
			return "Most of the color has been scratched off this once green metal box.";
		}
		else {
			return "Most of the color has been scratched off this once green metal box. You remember it vaguely, but exactly what is inside it eludes you. It is quite heavy and when you shake it carefully, it rattles. It has a simple lock, which at the moment seems locked.";
		}
	}

	function afterPick(gc) {
		that.isPicked = true;
		addVerb(metalBox, "close");
		gc.result = "Using the paper clip, you manage to open the lock. You open the box and it turns out to contain your old rock collection. Not much of a collection actually, a few small stones and your old rock pick.";
	}
})();
...
var cabinet = (function() {
	...
	function examine(gc) {
		if (hasVerb(cabinet, "close")) {
			return "You remember a lot of this stuff from your childhood. Many a time have you intended to go through this stuff, keeping some of it and throwing away some. Likely, you would throw away most of it, which may be the reason why you haven't gotten around to the task."
		}
		else {
			return "This old cabinet takes up a large portion of the wall."
		}
	}
})();
