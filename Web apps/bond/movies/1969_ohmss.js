const ohmss = {
    name: "On Her Majesty's Secret Service",
    year: 1969,
    scenes : [
        {
            // 0:00:00
            title: "Gun-barrel sequence",
            description: "The movie opens with the gun-barrel sequence, in which George Lazenby, against director Peter Hunt's will, kneels when firing his gun.",
        },
        { 
            // 0:00:40
            title: "Universal Exports (London) Ltd",
            description: 'Here, we see Big Ben reflected in the "Universal Exports" sign. Given the building on the right, and the trees on the left, the sign is likely positioned somewhere along [Great George Street|univex]. The rest of these scenes are recorded in the studio.',
        },
        { 
            // 0:01:20
            title: "Bond rescues Tracy", 
            description: "The scene starts with [Bond driving through Cascais|road_from_hotel], about 30 km west of Lisbon, in his Aston Martin DBS. We then see him driving along the coast. [Here|N247], he is overtaken by Tracy in her red Mercury Cougar XR7. Bond arrives at [Guincho Beach|guincho] where Tracy tries to kill herself. Bond rescues her and then has to fight Draco's goons.",
        },
        {
            // 0:06:34
            title: "Theme song",
            description: "The theme contains scenes from previous Bond films, probably in an attempt to establish this film's connection to the previous ones, regardless of the fact that we now have a new Bond.",
        },
        {
            // 0:08:58
            title: "Bond rescues Tracy... again",
            description: "We see bond returning [the same way|road_from_hotel] and then stopping outside [his hotel|palacio] - the Palacio Estoril - where Tracy's car is already parked. Later on, Bond rescues Tracy at the card table, fights off Draco's goons again and goes to bed with Tracy. The images from the lobby are said to have been recorded in the hotel. Most other indoor scenes are probably recorded in the studio.", 
        },
        {
            // 0:19:00
            title: "Bond meets Draco",
            description: "Bond is taken, at gun-point, from his hotel. Along the way, Bond is driven over the [25th of April Bridge|pte25apr] in Lisbon. When they arrive at the meeting, we see a rough industrial landscape. The interior scenes, Bond meeting Draco, are shot in a studio.",
        },
        {
            // 0:25:35
            title: "Bond tries to resign",
            description: "Back in London, Bond is relieved from operation Bedlam. Upset with this decision, he makes Moneypenny take a memo in which he requests his resignation from her Majesty's secret service. She changes its contents so that he in fact requests two weeks leave, a decision both Bond and M are grateful for. All scenes are recorded in the studio."
        },
        {
            // 0:29:15
            title: "We meet again",
            description: "The scene opens with Tracy [arriving|draco_entrance] at her father's estate for his birthday party. Bond is there as well, enjoying the [bullfighting|bullfight] spectacle put on by Draco. ",
        },
        { 
            // 0:33:54  
            title: "Falling in love",
            description: "Bond and Tracy fall in love. In a number of scenes, we see them walking in [a park|love_marqueses] and on [a beach|love_beach], and visiting [a jeweller's store|love_jewelers], presumably looking at wedding rings.", 
        },
        { 
            // 0:35:10  
            title: "A visit to Bern",
            description: "Draco takes Bond and Tracy to Bern. They pass the [Bärengraben|barengraben], an old bear pit that still houses bears,and drive across the [Nydeggbrücke|nydeggbrucke]. We also get a quick glimpse of the bell tower of the [Heiliggeistkirche|heiliggeistkirche] (Church of the Holy Spirit) on the way to [solicitor Gumbolds office|gumbold], which is in reality the Schweizerhof hotel. Parts of the interior scenes - specifically Bond entering the elevator - also seem to have been filmed here. The hotel has since been renovated, but the elevator and the stairwell next to it can still be clearly seen. [The entrance to solicitor Gumbold's office|gumbold_entrance] is not the Schweizerhof hotel, but an adjacent building. The Café Brésil next door is still (2018) there. [Bern's main railway station|bern_hbf] was being rebuilt during filming, and one of the cranes used featured in the film - the device Bond uses to crack Gumbold's safe is transported using it. While Bond is investigating Gumbold's office, Draco and Tracy have a discussion in the car. We can clearly see that this is taken in the [Kramgasse|kramgasse] (\"Grocer's Alley\"), today a partially pedestrian street. The clock tower in the background is the \"Zytglogge\", and we can also see the Simsonbrunnen (Samson fountain).", 
        },
        { 
            // 0:42:00
            title: "Visiting M",
            description: "Bond [arrives|m_entrance] at [M's house|m_house], informing him of the new development in the Blofeld case. M reinstates Bond, letting him get on with his investigation. The house in Marlow, about ten miles from Pinewood, was suggested by set decorator Peter Lamont, who lived nearby. An impressive building, the house that is supposed to belong to M has unfortunately burnt down since the movie was recorded. It has been replaced by a more modern, but still imposing, building.", 
        },
        {
            // 0:43:25
            title: "At the College of Arms",
            description: "Bond visits sir Hillary Bray at the College of Arms. In the first shot, we can clearly see [St. Paul's Cathedral|st_paul] in the background. [The College of Arms|arms] plays its correct role in the film. According to information (2018) on their web site, \"the College of Arms is the official heraldic authority for England, Wales, Northern Ireland and much of the Commonwealth including Australia and New Zealand\"."
        },
        {
            // 0:45:40
            title: "Arriving in Switzerland",
            description: "Bond arrives at the [Lauterbrunnen railway station|lauterbrunnen] and is greeted by Irma Bunt and Grunther. We can see clearly how they drive up to the main road and head south. The house we see on the left as their sled runs up the hill is the Gemeindehaus Adler, the Lauterbrunnen town hall. He is then taken to the helicopter. This scenery is rather a long way from Lauterbrunnen. In [one scene|heli_arrive], we see the sled arriving, and the three huts in the background, looking east, are still (2018) clearly visible. By helicopter, he is taken to  Blofeld's allergy research institute. While flying there, the helicopter passes above the Schilthorn [cable car station at Birg|birg_view]. We clearly see the outdoor restaurant.",
        },
        {
            // 0:50:55
            title: "Bond meets Blofeld and the girls",
            description: "The story of the [unfinished restaurant|piz], discovered by the film team, has been told many times. One of the most iconic locations in the whole franchise, the finalization of the restaurant was made possibly with money from the production company. Most of the scenes, interior and exterior, were recorded here."
        },
        {
            // 1:10:20
            title: "Campbell is denied access",
            description: "The scene where Campbell is denied entry by Grunther is likely recorded at the Schilthorn cable car station at [Birg|birg]", 
        },
        {
            // 1:11:40
            title: "Trouble at Piz Gloria",
            description: "Campbell is killed, Bond is exposed, the girls are sent away, and Bond escapes. Most of these scenes are recorded at [Piz Gloria|piz].",
        },
        {
            // 1:29:25
            title: "Escape from Piz Gloria",
            description: "Bond escapes down the mountains, followed by Blofeld and his henchmen."
        },
        {
            // 1:34:20
            title: "Civilization at last",
            description: "When Bond has escaped down the mountain, he arrives in the small town of [Grindelwald|grindelwald], over ten miles east of Piz Gloria, meaning that he would most likely have passed the town of Lauterbrunnen, where he arrived in Switzerland. In the first shot of the town, seen from up the mountain, we can clearly see a large house, today's [Grand Hotel Regina|regina] (2018). Irma Bunt sees the girls off at what is most likely the Schilthorn cable car station at [Stechelberg|stechelberg], given that the sign at the entrance says \"Gimmelwald Mürren Birg\". Bond fights some of the henchmen in what seems to be a storage area for bells, a fight that makes a horrible noise. Bond then enters the [ice rink|ice_rink] near hotel Regina in Grindelwald, an ice rink that is no longer there. Irma Bunt is seen outside a hotel, telling her goons to \"come on\". According to [this video|https://www.youtube.com/watch?v=agby3qCQ6YA], this could be current (2014) [hotel Bernerhof|bernerhof]. In these scenes, we clearly see a \"Hotel Oberland\" as well, but I haven't been able to locate that. There is a hotel Oberland in Lauterbrunnen. Possibly, they have recorded sequences for this scene in both places. At the ice rink, Bond meets Tracy again and they escape in her car.", 
        },
        {
            // 1:39:20
            title: "Trying to contact London",
            description: "When Bond and Tracy escape by car, Bond's intention is to try to contact London via telephone. These scenes are recorded in Lauterbrunnen. When they enter the town, we see an overview shot of the [church|l_church]. Bond tries to contacts London but gets interrupted. In the scene where Bond tries to reach London, [hotel Staubbach|staubbach] can be clearly seen in the background. The [phone booth|phone_booth] used by Bond is no longer there. When escaping after being shot at, Bond and Tracy pass the hotel [Jungfrau|jungfrau]. After passing the hotel Jungfrau, Bond and Tracy appear in a [crossing|crossing] which in reality is in the other direction, and more likely the one they would have passed earlier, on their way to the phone booth.", 
        },
        {
            // 1:41:25
            title: "The stock car race",
            description: "The [stock car race|stock_car] was filmed in a field outside Lauterbrunnen. The track was constructed by flooding a field with water and then letting it freeze.", 
        },
        {
            // 1:43:35
            title: "The proposal",
            description: "Bond and Tracy take cover in a [barn|barn] when they run out of petrol. Bond proposes to Tracy. The next day, [Blofeld and his men arrive|hillside], only to find the barn abandoned.", 
        },
        {
            // 1:48:50
            title: "The avalanche",
            description: "Blofeld and his men chase Bond and Tracy. According to mi6 declassified, issue 7, this scene is recorded somewhere in the vicinity of [Winteregg|winteregg], and photographed by Willy Bogner, who would later appear as a henchman in \"For your eyes only\". It ends with Bond and Tracy getting caught in an avalanche, after which Tracy is taken by Blofeld's men. Bond miraculously survives.", 
        },
        {
            // 1:53:22
            title: "Back in London",
            description: "Here, Bond is obviously upset with Tracy's disappearance and M informs Bond that the Government intend to give Blofeld complete amnesty for his crimes in exchange for the virus. Bond suggests a raid on Piz Gloria but this is denied.", 
        },
        {
            // 1:55:25
            title: "The raid on Piz Gloria",
            description: "Together with Draco's men, Bond instigates an attack on Piz Gloria, where Blofeld is holding Tracy. On the way, they communicate with [Zürich flight control|zurich_airport]. The attack is filmed at [Piz Gloria|piz] and ends with Blofeld and Bond jumping out from the nearest cable car tower, which has been covered with wooden walls.",
        },
        {
            // 2:05:45
            title: "The bob sleigh escape",
            description: "According to mi6 declassified, issue 7, the bob sleigh scene is recorded near Mürren, in an existing run that had been closed down in 1937 due to being too dangerous. Today there is [a run in Mürren|bob_run] that claims connections to the film.", 
        },
        {
            // 2:09:35
            title: "The wedding",
            description: "This scene starts with a short sequence where Bond enters [the jeweller's store|love_jewelers] to buy a wedding ring for Tracy. We then see the actual wedding, which was recorded at the [DeVinho estate in Zambujal|draco]", 
        },
        {
            // 2:12:20
            title: "Tragedy",
            description: "Bond stops the car at the side of the [road|tragedy] to remove the flowers, and Blofeld and Irma Bunt drive by. Irma Bunt shoots and kills Tracy.", 
        },
        {
            // 2:15:00 - 2:15:40
            title: "End credits",
            description: "\"Made by EON PRODUCTIONS LTD on location in SWITZERLAND and PORTUGAL and at PINEWOOD STUDIOS, LONDON, ENGLAND\"", 
        },
    ],
    locations: [
        {
            id: "univex",
            position: {
                lat: 51.501214, 
                lng: -0.127420,
                zoom: 17,
            },
            streetView: {
                zoom: 1,
                heading: 90,
                pitch: 0,
            },
            description: "\"Universal exports\" sign.",
        },
        {
            id: "road_from_hotel",
            position: {
                lat: 38.694550, 
                lng: -9.419931,
            },
            streetView: {
            },
            name: "Parada da Artilharia Anti Aerea / Av. Dom Carlos I",
            description: "Road by the sea.",
            sources: ["https://www.onthetracksof007.com/maps"],
        },
        {
            id: "N247",
            name: "N247 road",
            position: {
                lat: 38.764472, 
                lng: -9.470995,
            },
            streetView: {
                heading: 200,
            },
            description: "This is where Tracy overtakes Bond on her way to the beach.",
            sources: ["https://www.onthetracksof007.com/maps"],
        },
        {
            id: "guincho",
            name: "Praia Grande do Guincho",
            position: {
                lat: 38.732384, 
                lng: -9.473209,
            },
            streetView: {
                lat: 38.7298129,
                lng: -9.4724117,
                heading: 300,
            },
            description: "This is where Bond rescues Tracy di Vicenzo from the sea and fights Draco's goons.",
            sources: ["mi6 declassified # 7, page 26|http://www.mi6magazine.com"], 
        },
        {
            id: "palacio",
            name: "Palácio Estoril Hotel",
            position: {
                lat: 38.704813, 
                lng: -9.396409,
                zoom: 17,
            },
            mapView: {
                lat: 38.704803,
                lng: -9.396327,
                heading: 90,
                zoom: 19,
            },
            description: "Bond and Tracy stay at the Palacio hotel.",
            sources: [
                "http://www.movie-locations.com/movies/o/On-Her-Majestys-Secret-Service.php", 
                "http://www.jamesbondlifestyle.com/product/hotel-pal%C3%A1cio-estoril-portugal"
            ],
        },
        {
            id: "pte25apr",
            name: "Ponte 25 de Abril",
            position: {
                lat: 38.690775, 
                lng: -9.177354,
            },
            streetView: {
            },
            description: "When taken to Draco, Bond is driven over the 25th of April Bridge in Lissabon.",
        },
        {
            id: "draco_entrance",
            movieName: "The home of Marc-Ange Draco (entrance road)",
            name: "DeVinho estate",
            position: { 
                lat: 38.566376, 
                lng: -8.729593 
            },
            mapView: {
                // TODO
            },
            description: "This road is used by Tracy when she arrives at her father's estate.",
            sources: ["mi6 declassified # 7, page 26|http://www.mi6magazine.com"], 
        },
        {
            id: "bullfight",
            movieName: "The home of Marc-Ange Draco (bullfighting arena)",
            name: "DeVinho estate",
            position: { 
                lat: 38.567394, 
                lng: -8.724836,
            },
            mapView: {
                // TODO
            },
            description: "This is the bullfighting arena where the scenes for Marc-Ange Draco's birthday party were shot.",
            sources: ["mi6 declassified # 7, page 26|http://www.mi6magazine.com"], 
        },
        {
            id: "love_marqueses",
            name: "Palácio dos Marqueses da Fronteira",
            position: { 
                lat: 38.740097, 
                lng: -9.179777,
            },
            streetView: {
            },
            description: "Bond and Tracy walk in this park as they are falling in love.",
        },
        {
            id: "love_beach",
            name: "Private beach",
            position: { 
                lat: 38.523848, 
                lng: -8.738378,
            },
            description: "Bond and Tracy walk on this beach as they are falling in love.",
            sources: ["https://www.onthetracksof007.com/maps"],
        },
        {
            id: "love_jewelers",
            name: "Joalharia Ferreira Marques",
            position: { 
                lat: 38.712922, 
                lng: -9.139027,
            },
            streetView: {
                lat: 38.7131389,
                lng: -9.1390499,
                heading: 190,
                zoom: 3,
            },
            description: "The jeweller's store where Bond and Tracy look at rings.",
        },
        {
            id: "barengraben",
            name: "Bärengraben",
            position: { 
                lat: 46.948070, 
                lng: 7.459631,
            },
            mapView: {
            },
            description: "The car takes Bond and Tracy past the Bärengraben, an old bear pit that still houses bears.",
        },
        {
            id: "nydeggbrucke",
            name: "Nydeggbrücke",
            position: { 
                lat: 46.948478, 
                lng: 7.458368,
            },
            streetView: {
                lat: 46.948451,
                lng: 7.458682,
                heading: 270,
            },
            description: "The car takes Bond and Tracy across the Nydeggbrücke into the old city of Bern.",
        },
        {
            id: "heiliggeistkirche",
            name: "Heiliggeistkirche",
            position: { 
                lat: 46.948127, 
                lng: 7.440724,
            },
            streetView: {
                lat: 46.948980, 
                lng: 7.440613,
                heading: 180,
            },
            description: "We get a quick glimpse of the bell tower of the Heiliggeistkirche (Church of the Holy Spirit).",
        },
        {
            id: "gumbold",
            movieName: "Solicitor Gumbold's office",
            name: "Hotel Schweizerhof",
            position: { 
                lat: 46.948629, 
                lng: 7.440921,
            },
            streetView: {
                lat: 46.9487414,
                lng: 7.4406734,
                heading: 80,
                pitch: 20,
            },
            description: "Solicitor Gumbold's office.", 
            source: "https://www.youtube.com/watch?v=agby3qCQ6YA",
        },
        {
            id: "gumbold_entrance",
            movieName: "Solicitor Gumbold's office (entrance)",
            name: "House on Bollwerk 15, Bern",
            position: { 
                lat: 46.949359, 
                lng: 7.440662,
            },
            streetView: {
                lat: 46.9493792,
                lng: 7.4404928,
                heading: 110,
            },
            description: "The entrance to solicitor Gumbold's office.",
        },
        {
        id: "bern_hbf",
            name: "Bern Bahnhof",
            position: { 
                lat: 46.948940, 
                lng: 7.440281,
            },
            streetView: {
                lat: 46.9487414,
                lng: 7.4406734,
                heading: 280,
                pitch: 10,
            },
            description: "Bern's main railway station.",
            source: "https://www.bernerzeitung.ch/kultur/kino/Bern-im-BondFilm--damals-und-heute/story/30649899",
        },
        {
            id: "kramgasse",
            name: "Kramgasse",
            position: { 
                lat: 46.947979, 
                lng: 7.450369,
            },
            streetView: {
                lat: 46.9479457,
                lng: 7.4501483,
                heading: 270,
            },
            description: "Kramgasse"
        },
        {
            id: "m_entrance",
            name: "Thames Lawn Estate (entrance)",
            movieName: "Entrance to M's house",
            position: { 
                lat: 51.568250, 
                lng: -0.771983,
            },
            streetView: {
                heading: 30,
            },
            description: "Bond enters M's house through an imposing gate. The gate remains today.",
            sources: ["mi6 declassified # 7, page 26|http://www.mi6magazine.com"], 

        },
        {
            id: "m_house",
            name: "Thames Lawn Estate",
            movieName: "M's house",
            position: { 
                lat: 51.568363, 
                lng: -0.770401,
            },
            mapView: {

            },
            description: "M's house.",
            sources: ["mi6 declassified # 7, page 26|http://www.mi6magazine.com"], 
        },
        {
            id: "st_paul",
            name: "St. Paul's Cathedral",
            position: { 
                lat: 51.513766, 
                lng: -0.098320,
            },
            mapView: {
                // TODO
            },
            description: "St. Paul's Cathedral",
        },
        {
            id: "arms",
            name: "The College of Arms",
            position: { 
                lat: 51.512152, 
                lng: -0.098734,
            },
            streetView: {
                lat: 51.5119807,
                lng: -0.0987014,
            },
            description: "The College of Arms",
        },
        {
            id: "lauterbrunnen",
            name: "Lauterbrunnen, railway station",
            position: { 
                lat: 46.598314, 
                lng: 7.907782, 
            },
            streetView: {
                lat: 46.5976,
                lng: 7.9074303,
                heading: 45,
                pitch: -20,
                zoom: 1,
            },
            description: "Lauterbrunnen", 
        },
        {
            id: "heli_arrive",
            name: "Bond arrives at the helicopter",
            position: { 
                lat: 46.639905, 
                lng: 8.055360,
            },
            streetView: {
                lat: 46.6398652,
                lng: 8.0556495,
                heading: 130,
                zoom: 2,
            },
            description: "Helicopter"
        },
        {
            id: "birg_view",
            name: "Birg",
            position: { 
                lat: 46.561823, 
                lng: 7.857703,
            },
            // TODO picture
            description: "Birg",
        },
        {
            id: "piz",
            name: "Piz Gloria",
            position: { 
                lat: 46.557227, 
                lng: 7.835204,
            },
            imageView: {
                title: "By Yesuitus2001 [CC BY-SA 2.5 (https://creativecommons.org/licenses/by-sa/2.5)], from Wikimedia Commons",
                href: "https://commons.wikimedia.org/wiki/File:Piz-Gloria093.JPG",
                alt: "Piz-Gloria093",
                src: "https://upload.wikimedia.org/wikipedia/commons/thumb/a/a0/Piz-Gloria093.JPG/512px-Piz-Gloria093.JPG",
            },
            description: "Piz Gloria",
        },
        {
            id: "birg",
            name: "Birg",
            position: { 
                lat: 46.561823, 
                lng: 7.857703,
            },
            description: "Birg",
            confirmed: false,
        },
        {
            // TODO circle
            id: "grindelwald",
            name: "Grindelwald",
            position: { 
                lat: 46.624077, 
                lng: 8.033693,
                zoom: 17,
            },
            description: "Grindelwald",
        },
        {
            id: "regina",
            name: "Hotel Regina",
            position: { 
                lat: 46.624077, 
                lng: 8.033693,
                zoom: 17,
            },
            streetView: {
                lat: 46.6242404,
                lng: 8.0348341,
                heading: 270,
                zoom: 2,
            },
            description: "Hotel Regina",
        },
        {
            id: "stechelberg",
            name: "Stechelberg",
            position: { 
                lat: 46.555083, 
                lng: 7.901748,
            },
            streetView: {
                lat: 46.5553092,
                lng: 7.9017925,
                heading: 90,
            },
            description: "Schilthorn cable car station at Stechelberg",
        },
        {
            id: "ice_rink",
            name: "Ice rink",
            position: { 
                lat: 46.624304, 
                lng: 8.034437,
                zoom: 17,
            },
            streetView: {
                lat: 46.6242404,
                lng: 8.0348341,
                heading: 270,
                zoom: 2,
            },
            description: "The ice rink where Bond meets Tracy again.",
        },
        {
            id: "bernerhof",
            name: "Hotel Bernerhof",
            position: { 
                lat: 46.624402, 
                lng: 8.034372,
                zoom: 17,
            },
            streetView: { 
                lat: 46.6244068,
                lng: 8.034429, 
            },
            description: "Hotel Bernerhof",
        },
        {
            id: "l_church",
            name: "Lauterbrunnen church",
            position: { 
                lat: 46.592701, 
                lng: 7.908466,
            },
            description: "We see an overview shot of the church in Lauterbrunnen.",
        },
        {
            id: "staubbach",
            name: "Hotel Staubbach",
            position: { 
                lat: 46.593282, 
                lng: 7.907721,
            },
            description: "Hotel Staubbach",
        },
        {
            id: "phone_booth",
            name: "Phone booth",
            position: { 
                lat: 46.593640, 
                lng: 7.907140,
            },
            description: "Phone booth",
        },
        {
            id: "jungfrau",
            name: "Hotel Jungfrau",
            position: { 
                lat: 46.593942, 
                lng: 7.907142,
            },
            description: "Hotel Jungfrau",
        },
        {
            id: "crossing",
            name: "Road crossing",
            position: { 
                lat: 46.592858, 
                lng: 7.907516,
            },
            description: "Crossing",
        },
        {
            id: "stock_car",
            name: "Stock car race",
            position: { 
                lat: 46.583531, 
                lng: 7.913499,
            },
            description: "This is where the stock car race was filmed.",
            source: ["https://www.onthetracksof007.com/maps", "https://007travelers.blogspot.com/2014/12/007-travel-story-lauterbrunnen.html",
            "mi6 declassified # 7, page 26|http://www.mi6magazine.com"],
        },
        {
            id: "barn",
            name: "Barn",
            position: { 
                lat: 46.755356, 
                lng: 7.686779,
            },
            description: "Barn",
        },
        {
            id: "hillside",
            name: "Hillside", 
            position: { 
                lat: 46.754188, 
                lng: 7.687437,
            },
            description: "Blofeld and his men arrive"
        },
        {
            id: "winteregg",
            name: "Winteregg", 
            position: { 
                lat: 46.581872, 
                lng: 7.896059,
            },
            description: "In this region, the hunt for Bond and Tracy was filmed."
        },
        {
            id: "zurich_airport",
            name: "Zurich airport", 
            position: { 
                lat: 47.458376, 
                lng: 8.555497,
            },
            description: "Zurich airport",
            category: "unseen",
        },
        {
            id: "bob_run",
            name: "Bob sleigh run", 
            position: { 
                lat: 46.563826, 
                lng: 7.889260,
            },
            description: "Bob sleigh run",
        },
        {
            id: "draco",
            name: "Draco's residence", 
            position: { 
                lat: 38.566969, 
                lng: -8.730118,
            },
            description: "The wedding party takes place on Draco's residence."
        },
        {
            id: "tragedy",
            name: "Tragedy", 
            position: { 
                lat: 38.480622, 
                lng: -8.989766,
            },
            description: "Tracy gets killed by Irma Bunt.",
        },
    ]
}