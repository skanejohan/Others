...
abstract class TweetSet {
   ...
   */
  def filter(p: Tweet => Boolean): TweetSet = filter0(p, new Empty) //Johan, Filtering
  ...
  // Johan, sorting
  def ascendingByRetweet: Trending = ascendingByRetweet1(new EmptyTrending)
  def ascendingByRetweet1(accu: Trending): Trending =
    if(this.isEmpty) accu
    else this.remove(this.findMin).ascendingByRetweet1(accu + this.findMin)
...
object GoogleVsApple {
  ...
  val googleTweets: TweetSet = TweetReader.allTweets.filter(tweet => google.exists(t => tweet.text.contains(t))) // Johan, tying together

  val appleTweets: TweetSet = TweetReader.allTweets.filter(tweet => apple.exists(t => tweet.text.contains(t))) // Johan, tying together

  // Q: from both sets, what is the tweet with highest #retweets?
  val trending: Trending = googleTweets.union(appleTweets).ascendingByRetweet // Johan, tying together
}
