...
package object nodescala {
  ...
  implicit class FutureCompanionOps[T](val f: Future.type) extends AnyVal {
    ...
    def any[T](fs: List[Future[T]]): Future[T] = {
      val p = Promise[T]()
      fs map (elem => elem onComplete { case x => p.tryComplete(x) })
      p.future
    }
  ...
  implicit class FutureOps[T](val f: Future[T]) extends AnyVal {
    ...
    def now: T = (f value) match {
      case Some(Success(success)) => success
      case Some(Failure(failure)) => throw new NoSuchElementException
    ...
    def continueWith[S](cont: Future[T] => S): Future[S] = {
      val p = Promise[S]()
      f onComplete {
        case Failure(_) => p failure (new Exception)
        case Success(_)    => try {
          p success (cont (f))
        } catch {
